...
UI_EVENT_TO_MODEL = {
    "None": None,
    "Port Strike": "Red Sea / Suez rerouting crisis",
    "Canal Disruption": "Panama Canal drought restrictions",
    "Monsoon Season": "Indian monsoon season port slowdown",
    "High Demand": "China steel demand slump",
}

MODEL_EVENT_TO_UI = {
    "Red Sea / Suez rerouting crisis": "Port Strike",
    "Panama Canal drought restrictions": "Canal Disruption",
    "Indian monsoon season port slowdown": "Monsoon Season",
    "China steel demand slump": "High Demand",
}
...
class FreightEngine:
    ...
    def _safe_encode(self, col: str, value: str) -> int:
        ...
        if value not in le.classes_:
            raise ValueError(
                f"Invalid {col} category: '{value}'. "
                f"Valid categories are: {list(le.classes_)}"
            )
    ...
    def forecast_14d(
        self,
        base_params: dict,
        bdi_trend: float = 0.003,
        fuel_trend: float = 0.001,
        ...
        for d in range(14):
            ...
            p = base_params.copy()
            p["baltic_style_index"] = (
                p["baltic_style_index"] * ((1 + bdi_trend) ** d)
            )
            p["bunker_fuel_price"] = (
                p["bunker_fuel_price"] * ((1 + fuel_trend) ** d)
            )
...
class FuelEngine:
    ...
    def optimize_speed(
        ...
        for speed in np.arange(
            ...
            distance = float(
                vessel_data["distance_nm"].iloc[0]
            )
        ...
        feasible = df_r[
            df_r["voyage_hours"] <= required_hours
        ]

        if feasible.empty:
            best = df_r.loc[
                df_r["total_cost"].idxmin()
            ].to_dict()
            best["feasible"] = False
        else:
            best = feasible.loc[
    ...
    def forecast_bunker_14d(
        self,
        base_price: float,
        daily_drift: float = 0.0012,
    ) -> dict:
...
class TimingEngine:
    ...
    def __init__(self) -> None:
        csv_path = (
            _TIMING_DIR
            / "freight_ship_fuel_sample_1000 (1).csv"
        )
        ...
        current_h = df.loc[
            X_test.index,
            "current_charter_rate",
        ].values

        self.directional_accuracy = float(
            np.mean(
                (y_pred > current_h)
                == (y_test.values > current_h)
            )
            * 100
        )

        self.mae_holdout = float(
            np.mean(
                np.abs(
                    y_pred
                    - y_test.values
    ...
    def get_decision(
        ...
        if current_rate <= penalised_future:
            ...
            decision_label = (
                "EXECUTE FIXTURE — LOCK RATE NOW  [T+0]"
            )
            ...
            decision_label = (
                "DECISION RECOMMENDATION: DEFER FIXTURE  "
                f"(T+{t_star} TARGET)"
            )
        ...
        return {
            ...
            "predicted_future": round(
                predicted_future,
                2,
            ),
            "penalised_future": round(
                penalised_future,
                2,
            ),
            "optimal_t_star": t_star,
            "savings_if_wait": round(
                savings_if_wait,
                2,
            ),
            "daily_rates": [
            ...
            "bunker_surcharge": round(
                bunker_surcharge,
                2,
            ),
            "inventory_hold": round(
                inventory_hold,
                2,
            ),
...
class VesselEngine:
    ...
    def rank(
        ...
        if fuel_predictions is not None:
            df["fuel_consumed_tpd"] = (
                fuel_predictions.values
            )
        ...
        def _is_suitable(
            ...
            if float(v["draft"]) > max_draft:
                return (
                    False,
                    f"Draft {v['draft']:.1f}m > "
                    f"port limit {max_draft}m",
                )

            if voyage_days > required_days:
                return (
                    False,
                    "Exceeds transit window",
                )

            return True, "Suitable"

        suitability = df.apply(
            _is_suitable,
            axis=1,
            result_type="expand",
        )
        ...
        suited["fuel_efficiency"] = (
            suited["fuel_consumed_tpd"]
            / suited["dwt"]
        )

        suited["demurrage_exposure"] = (
            queue_days
            * demurrage_day
        )

        suited["capacity_score"] = (
            self._norm_hi(
                suited["dwt"]
            )
        )
...
def run_optimization(params: dict) -> dict:
    ...
    # ── 1. Freight Forecast ─────────────────────────────────────────────────
    model_cargo_type = UI_CARGO_TO_MODEL.get(
        params["cargo_type"],
        params["cargo_type"],
    )

    model_market_event = UI_EVENT_TO_MODEL.get(
        params.get("market_event", "None"),
        params.get("market_event"),
    )

    freight_params = {
        "route": params["route"],
        "cargo_type": model_cargo_type,
        "vessel_class": params[
            "vessel_class"
        ],
        "market_event": model_market_event,
        "vessel_dwt": params[
            "vessel_dwt"
        ],
        "distance_nm": params[
        ...
        "baltic_style_index": params[
            "baltic_style_index"
        ],
        "bunker_fuel_price": params[
            "bunker_fuel_price"
        ],
    ...
    vessel_row = pd.DataFrame([{
        "vessel_type": _dwt_to_type(
            params["vessel_dwt"]
        ),
        "dwt": params["vessel_dwt"],
        "engine_power": 15_000,
        "vessel_age": params.get(
            "vessel_age",
            8,
        ),
...
if __name__ == "__main__":
    ...
    print(
        "Net Savings: ${:,.0f}".format(
            result["net_savings_usd"]
        )
    )

    print(
        "TLC Breakdown:",
        result["tlc_breakdown"],
    )

    print(
        "Optimal T*: T+{}".format(
            result["optimal_t_star"]
        )
    )